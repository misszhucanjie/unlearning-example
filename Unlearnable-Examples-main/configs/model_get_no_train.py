...
noise = np.load('./data_save/noise_posion')
...
label_index = [1,2,3,4,5,6,7]
for i in range(len(unlearnable_train_dataset)):
    for index in range(len(unlearnable_train_dataset.data[i])):
        if unlearnable_train_dataset.label[i][index] in label_index:
            unlearnable_train_dataset.data[i][index] += perturb_noise[i][index]
            unlearnable_train_dataset.data[i][index] = np.clip(unlearnable_train_dataset.data[i][index], a_min=0, a_max=255)
