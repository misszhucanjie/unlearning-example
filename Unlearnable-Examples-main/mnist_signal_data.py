...
clean_train_dataset = datasets.MNIST(root='../datasets', train=True, download=True, transform=ToTensor())
# clean_train_dataset = datasets.CIFAR10(root='../datasets', train=False, download=True, transform=test_transform)
clean_test_dataset = datasets.MNIST(root='../datasets', train=False, download=True, transform=ToTensor())

clean_train_loader = DataLoader(dataset=clean_train_dataset, batch_size=64,
                                shuffle=True, pin_memory=True,
                                drop_last=False, num_workers=12)
clean_test_loader = DataLoader(dataset=clean_test_dataset, batch_size=64,
                                shuffle=True, pin_memory=True,
                                drop_last=False, num_workers=12)
...
while condition:
    ...
    for i, (images, labels) in enumerate(clean_train_loader):
        image_clean = copy.deepcopy(images)
        for i, _ in enumerate(images):
            # Update noise to images
            #
            ...
            eval_idx += 1
        images, labels,image_clean = images.cuda(), labels.cuda(),image_clean.cuda()
        with torch.no_grad():
            logits = base_model(images)
            logits_clean = base_model(image_clean)
    ...
    print('acc_clean %.2f' % (acc_clean * 100))
    if acc > 0.60:
        condition = False
    np.save('./data_save/mnist_noise_posion',noise)
    path = './mnist_point_best_module.pkl'
...
train_transform = transforms.Compose(train_transform)
clean_train_dataset = datasets.CIFAR10(root='../datasets', train=True, download=True, transform=ToTensor())
unlearnable_train_dataset = datasets.CIFAR10(root='../datasets', train=True, download=True, transform=ToTensor())
