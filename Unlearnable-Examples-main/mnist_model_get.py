...
noise = np.load('./data_save/mnist_noise_posion.npy')
...
clean_train_dataset_2 = datasets.CIFAR10(root='../datasets', train=True, download=True, transform=train_transform)
unlearnable_train_dataset_2 = datasets.CIFAR10(root='../datasets', train=True, download=True, transform=train_transform)

perturb_noise = noise.mul(255).clamp_(0, 255).permute(0, 2, 3, 1).to('cpu').numpy().squeeze(-1)
...
def get_pairs_of_imgs(idx):
    clean_img = np.array(clean_train_dataset.data[idx])
    unlearnable_img = np.array(unlearnable_train_dataset.data[idx])
...
selected_idx = [random.randint(0, 60000) for _ in range(10)]
img_grid = []
